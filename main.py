...
for i in page_numbers[4:5]:
  url = f'Wikipedia:Reliable_sources/Perennial_sources/{i}'
  wikicode = get_cached_page(site, url)
  table = wikicode.filter_tags(matches=lambda node: node.tag == 'table')[0]

  onlyinclude = table.contents.filter_tags(recursive=True, matches=lambda node: node.tag == 'onlyinclude')[0]
  ...
  for item in items:
      ...
      if i == 1:
          ...
          if not links:
              ...
              if not text:
                  raise IncompleteParseError(f"Could not find name in row: {item_wikicode}")
              if text:
          ...
          content_qualifier = ' '.join(RE_PARENTHESIZED.findall(item_wikicode.strip_code())).strip()
          ...
          data['discussion'] = ' '.join(str(l) for l in item_wikicode.filter_wikilinks())
      elif i == 5:
      ...
      if templates:
          for template in templates:
            if template.name == 'WP:RSPSTATUS':
                ...
                entry['rfc'] = template.has('rfc') and template.get('rfc').value.lower().startswith('y')
